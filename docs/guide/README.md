...
- Use a sub folder of a repo as dependency (master branch will be used)
  ...
  ```
  https://gitexec.now.sh/<repo>/<subdir>
  ```

- If you want to use another branch or commit instead

  ```
  https://gitexec.now.sh/<repo>/<subdir>?<commit-ish>
  ```
...
- In fact, usage without sub folder is also available:
  ...
  `https://gitexec.now.sh/<repo>`

  `https://gitexec.now.sh/<repo>?<commit-ish>`
...
# dep: repo=[neopilot/gitexec-hello] > sub folder=[packages/hello]
yarn add https://gitexec.now.sh/neopilot/gitexec-hello/packages/hello

# dep: [neopilot/gitexec-hello] > [packages/core] # branch=[feat/md]
yarn add https://gitexec.now.sh/neopilot/gitexec-hello/packages/core?feat/md
```

## More Formal API

```
https://gitexec.now.sh/api/pkg?url=<repo>/<subdir>
...
```
https://gitexec.now.sh/api/pkg.tgz?url=&commit=
```
...
- Simplest API
  ...
  ```
  https://gitexec.now.sh/<repo>/<subdir>?<commit-ish>&scripts.<script-name>=<script-content>
  ```

- More Formal API

  ```
  https://gitexec.now.sh/pkg?url=<repo>/<subdir>&scripts.<script-name>=<script-content>
...
- To **replace** the original, just use `scripts.postinstall=command-from-gitexec`,
  then the generated `package.json` will be like:

  ```json
  {
    "scripts": {
      "postinstall": "command-from-gitexec"
    }
  }
  ```

- To **append** to the original, add `&&` (encoded as `%26%26`) **before** your script content: `scripts.postinstall=%26%26command-from-gitexec`.
  Then the generated `package.json` will be like:

  ```json
  {
    "scripts": {
      "postinstall": "node original-install.js && command-from-gitexec"
    }
  }
  ```

- To **prepend** to the original, add `&&` **after** your script content: `scripts.postinstall=command-from-gitexec%26%26`.
  Then the generated `package.json` will be like:

  ```json
  {
    "scripts": {
      "postinstall": "command-from-gitexec && node original-install.js"
    }
  }
  ```
...
GitExec service process the tar file of the github repo in the form of stream,
...
GitExec service requests the tarball of whole repo `example-user/example-repo` from github,
...
This is how GitExec works.

## Examples

### Custom script example

This example shows how to install [neopilot/gitexec-hello > packages/hello-ts](https://github.com/neopilot/gitexec-hello/tree/master/packages/hello-ts) as dependency.
...
cd hello-gitexec
npm init -y
npm install 'https://gitexec.now.sh/neopilot/gitexec-hello/packages/hello-ts?master&scripts.postinstall=npm%20install%20--ignore-scripts%20%26%26%20npm%20run%20build'
```
