<template>
  <div
    class="gitexec-container"
    :class="{
      'gitexec-preview': preview,
    }"
  >
    <div class="input-container">
      <action-bar>
        ...
        <input
          class="gitexec-input main-input with-right"
          :class="{ error }"
          autofocus
          v-model="url"
        ...
        />
        <button
          class="gitexec-button icon with-left"
          type="button"
          @click="url = ''"
        >
...
.gitexec-container
  ...
  &.gitexec-preview > :not(.input-container),
  &.gitexec-preview > :not(.input-container) >>> *
    color lightgrey
